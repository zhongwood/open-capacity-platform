...
import com.open.capacity.user.dao.SysOrgDao;
...
public class SysUserServiceImpl implements SysUserService {
    ...
    @Override
    public PageResult<SysOrg> findAllOrg(Map<String, Object> params) {
        PageUtil.pageParamConver(params, true);
        ...
        List<SysOrg> list = sysOrgDao.findList(params);
        PageInfo<SysOrg> pageInfo = new PageInfo<>(list);
        return PageResult.<SysOrg>builder().data(pageInfo.getList()).code(0).count(pageInfo.getTotal()).build();
    }

    @Override
    public Result saveOrUpdate(SysOrg sysOrg) {
        String orgName = sysOrg.getOrgName();
        if (StringUtils.isBlank(orgName)) {
            //throw new IllegalArgumentException("用户名不能为空");
            return Result.failed("分组名不能为空");
        }
        int i = 0;
        if (sysOrg.getId() == null) {
            SysOrg persistenceUser = sysOrgDao.findOrgByName(orgName);
            if (persistenceUser != null) {
                //throw new IllegalArgumentException("用户名已存在");
                return Result.failed("该分组已存在");
            }
            sysOrg.setUpdateTime(DateUtils.getTimestamp());
            sysOrg.setCreateTime(DateUtils.getTimestamp());
            i = sysOrgDao.save(sysOrg);
        } else {
            sysOrg.setUpdateTime(DateUtils.getTimestamp());
            i = sysOrgDao.updateByOps(sysOrg);
        }
    ...
    public Result findAllOrgs() {
        ...
        return Result.succeed(sysOrgDao.findList(param), "操作成功");
    }
}